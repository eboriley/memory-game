...
io.on('connection', (socket) => {
  ...
  socket.on('join', function (roomno) {
    if (rooms.length >= 1) {
      const roomsLikeUserChoiceOfRoom = searchRooms(roomno, socket, io);
      if (!roomsLikeUserChoiceOfRoom) {
        createRoom(roomno, socket, io);
      } else addClientToChoiceOfRoom(roomsLikeUserChoiceOfRoom, socket, io);
    }
    if (rooms.length === 0) {
      createRoom(roomno, socket, io);
    }
  ...
  socket.on('message', (message) => {
    ...
    io.to(text.roomId).emit('message', `${text.id} said ${text.message}`);
  });
...
http.listen(8080, () => console.log('listening on port http://localhost:8080'));
...
function createRoom(roomno, socket, io) {
  ...
  io.to(room.id).emit('connectToRoom', 'you are in room no ' + room.id);
...
function addClientToChoiceOfRoom(roomsLikeUserChoiceOfRoom, socket, io) {
  if (roomsLikeUserChoiceOfRoom) {
    for (let room of rooms) {
